...
def sim_run( threadName, seed, hll_bits, grouped_hll, num_bits, filenames):
    ...
    for filename in filenames:
        ...
        for runs in range(0,10):
            ...
            fr2.write(str(runs) + "\t" + str(round(hll_st,3)) +"\t"+ str(round(q_st/float(hll_size),6))     +"\t"+ str(round(hll_st2,3)) +"\t"+ str(round(hll_st1,6)) +"\t"+ str(len(gelement)) + "\t" + str(tot_pkts) + "\t" + filename + "\t" + str(timer0.total_seconds()) + "\t" + str(timer1.total_seconds()) +  "\n")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='StreamHLL_Vec - PCAP sim') 
    # Positional argument
...
print(filenames)
ths = []
for hll_bits in [6, 8, 10, 12, 14]:
#for hll_bits in [10]:
    for grouped_hll in [2,4,8]:
        for num_bits in [3,4,5]:
            ths.append( Process(target = sim_run, args = ("Trace1", 192, hll_bits, grouped_hll, num_bits, filenames) ) )
            ths.append( Process(target = sim_run, args = ("Trace2",   2, hll_bits, grouped_hll, num_bits, filenames) ) )
            ths.append( Process(target = sim_run, args = ("Trace3", 342, hll_bits, grouped_hll, num_bits, filenames) ) )
            ths.append( Process(target = sim_run, args = ("Trace4", 992, hll_bits, grouped_hll, num_bits, filenames) ) )
            ths.append( Process(target = sim_run, args = ("Trace5",  92, hll_bits, grouped_hll, num_bits, filenames) ) )
